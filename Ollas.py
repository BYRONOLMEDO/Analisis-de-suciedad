# ============================================================
# COMPARADOR DE OLLAS (VIDRIO) – Colab, “detalle extremo”
# ============================================================
# Qué hace:
#   1) Te pide subir VARIAS OLLAS LIMPIAS (una sola acción).
#   2) Para CADA olla limpia, te pide subir VARIAS OLLAS SUCIAS asociadas.
#   3) Calcula un PUNTAJE DE SUCIEDAD (0 = limpio, 100 = más sucio)
#      pensado para vidrio, mitigando reflejos y variaciones de luz.
#   4) Muestra por cada olla limpia:
#        - Tabla ordenada de MÁS sucio a MÁS limpio (con nombres de archivo)
#        - Gráfico de barras
#        - CSV guardado en /content/resultados_ollas/
...
# 1) Subir OLLAS LIMPIAS
print("📤 Sube AHORA tus OLLAS LIMPIAS (puedes seleccionar varios a la vez).")
...
if not clean_images:
    raise SystemExit("No se cargaron ollas limpias. Vuelve a ejecutar y súbelos.")

print(f"✅ Ollas limpios cargados: {list(clean_images.keys())}")

# 2) Subir OLLAS SUCIAS POR CADA LIMPIO
sucios_por_limpio = {}
for clean_name in clean_images.keys():
    print(f"\n📤 Sube AHORA las OLLAS SUCIAS para: {clean_name} (puedes seleccionar varios).")
...
# Carpeta de salida
os.makedirs("resultados_ollas", exist_ok=True)
...
for clean_name, dirty_dict in sucios_por_limpio.items():
    ...
    for i, row in df.iterrows():
        print(f"  {i+1:02d}. {row['Olla Sucia']}  →  {row['Nivel de Suciedad (%)']}%")

    # Gráfico de barras
    plt.figure(figsize=(9, 4))
    plt.title(f"Suciedad por olla sucia vs {clean_name}")
    plt.bar(df["Olla Sucia"], df["Nivel de Suciedad (%)"])
    plt.xticks(rotation=45, ha="right")
    plt.ylim(0, 100)
    plt.ylabel("Nivel de Suciedad (%)")
    plt.xlabel("Ollas Sucias")
    plt.tight_layout()
    plt.show()

    # Guardar CSV por olla limpia
    base_clean = os.path.splitext(os.path.basename(clean_name))[0]
    out_csv = os.path.join("resultados_ollas", f"resultados_{base_clean}.csv")
...
if resumen_rows:
    resumen = pd.DataFrame(resumen_rows).sort_values(
        by=["Olla Limpia", "Nivel de Suciedad (%)"], ascending=[True, False]
    ).reset_index(drop=True)
    print("\n================== RESUMEN GENERAL (todas las comparaciones) ==================")
    display(resumen)

    resumen_path = os.path.join("resultados_ollas", "resumen_general.csv")
    resumen.to_csv(resumen_path, index=False)
    print(f"📁 CSVs guardados en: /content/resultados_ollas/")
else:
    print("\nNo se encontraron comparaciones. ¿Subiste sucios para cada olla limpia?")
